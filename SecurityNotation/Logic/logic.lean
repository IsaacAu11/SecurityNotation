...
section Knows   
  ...
  inductive Derives (knowledge_base : Message → Prop) : Principal → Message → Prop where
    | base : ∀ p m, knowledge_base m → Derives knowledge_base p m
  ...
  /- 
    The knows inductive function combines everything into one, allowing all the logic to be connected and
    showing what each principal knows and can figure out. allows us to see and chain together traces and messages.  
  -/
  inductive Knows (p : Principal) (t : Trace) : Message → Prop where
    | from_initial : ∀ m, Initial_knowledge p m → Knows p t m
    | from_trace   : ∀ m, Trace_Knowledge p t m → Knows p t m
    | from_derives : ∀ m, Derives (Knows p t) p m → Knows p t m
